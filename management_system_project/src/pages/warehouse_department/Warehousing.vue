    ...
    <el-table ref="multipleTable" :data="mockData" tooltip-effect="dark" style="width: 100%">
      ...
      <el-table-column label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="primary" @click="delEnd(scope.row)" class="btnDel">入库</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 入库
    delEnd(row) {
      this.$confirm("此操作将此采购单入库, 是否继续?", "提示", {
        ...
        .then(() => {
          axios
            .get(
              `api/main/stock/instock?poId=${row.poId}&type=${row.status}&page=${this.page}`
            )
            .then(resp => {
              this.changeData(resp);
              location.reload();
    ...
    showPoDetails(row) {
      this.visible = true;
      axios.get(`api/main/purchase/pomain/queryItem?poId=${row.poId}`).then(resp => {
        console.log(resp.data)
